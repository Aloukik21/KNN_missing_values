...
toyota.fuel_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.80, 0.20), size = length(cc), replace = TRUE) ]))#-----------------selecting only 5 percent values of na
...
accu_d <- accuracy(x)
write.csv(x,'W:\\cases\\Question 1\\20 percent values removed\\KNN weighted\\MyData_catagorical_knnw_20percent.csv', row.names = TRUE)
write.table(accu_d, 'W:\\cases\\Question 1\\20 percent values removed\\KNN weighted\\MyData_catagorical_knnw_20percent.csv', col.names=FALSE, sep=",", append=TRUE)
