...
toyota.age_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.80, 0.20), size = length(cc), replace = TRUE) ]))#------------------creating random nas
...
msse <- get_mse(xn)
write.csv(xn,'W:\\cases\\Question 1\\20 percent values removed\\1NN\\MyData_continuous_1nn_20percent.csv', row.names = TRUE)
write.table(msse, 'W:\\cases\\Question 1\\20 percent values removed\\1NN\\MyData_continuous_1nn_20percent.csv', col.names=FALSE, sep=",", append=TRUE)
