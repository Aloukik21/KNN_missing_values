...
toyota.fuel_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.95, 0.05), size = length(cc), replace = TRUE) ]))
...
accu_d <- accuracy(x)
write.csv(x,'W:\\cases\\Question 1\\5 percent values removed\\1NN\\MyData_catagorical_1nn_5percent.csv', row.names = TRUE)
write.table(accu_d, 'W:\\cases\\Question 1\\5 percent values removed\\1NN\\MyData_catagorical_1nn_5percent.csv', col.names=FALSE, sep=",", append=TRUE)
