...
toyota.age_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.95, 0.05), size = length(cc), replace = TRUE) ]))
...
msse <- get_mse(xn)
write.csv(xn,'W:\\cases\\Question 1\\5 percent values removed\\KNN weighted\\MyData_continuous_1nn_5percent.csv', row.names = TRUE)
write.table(msse, 'W:\\cases\\Question 1\\5 percent values removed\\KNN weighted\\MyData_continuous_1nn_5percent.csv', col.names=FALSE, sep=",", append=TRUE)
