...
toyota.fuel_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.95, 0.05), size = length(cc), replace = TRUE) ]))#-----------------selecting only 5 percent values of na
...
accuracy(xn)
print(paste("Accuracy is", accuracy(xn)))


accu_d <- accuracy(xn)
write.csv(x,'W:\\cases\\Question 1\\5 percent values removed\\KNN weighted\\MyData_catagorical_1nn_5percent.csv', row.names = TRUE)
write.table(accu_d, 'W:\\cases\\Question 1\\5 percent values removed\\KNN weighted\\MyData_catagorical_1nn_5percent.csv', col.names=FALSE, sep=",", append=TRUE)
