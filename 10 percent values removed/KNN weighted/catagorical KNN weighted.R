...
toyota.fuel_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.90, 0.10), size = length(cc), replace = TRUE) ]))#-----------------selecting only 5 percent values of na
...
toyota.missing_100<-as.data.frame(mat_bind_fuel[1:100,])#-----------------------------taking number of instances
...
accu_d <- accuracy(d)
write.csv(d,'W:\\cases\\Question 1\\10 percent values removed\\KNN weighted\\MyData_catagorical_1nn_10percent.csv', row.names = TRUE)
write.table(accu_d, 'W:\\cases\\Question 1\\10 percent values removed\\KNN weighted\\MyData_catagorical_1nn_10percent.csv', col.names=FALSE, sep=",", append=TRUE)
