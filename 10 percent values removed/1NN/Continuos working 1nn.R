...
toyota.age_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.90, 0.10), size = length(cc), replace = TRUE) ]))#------------------creating random nas
...
msse <- get_mse(xn)
write.csv(xn,'W:\\cases\\Question 1\\10 percent values removed\\1NN\\MyData_continuous_1nn_10percent.csv', row.names = TRUE)
write.table(msse, 'W:\\cases\\Question 1\\10 percent values removed\\1NN\\MyData_continuous_1nn_10percent.csv', col.names=FALSE, sep=",", append=TRUE)
