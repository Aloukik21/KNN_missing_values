...
toyota.fuel_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.90, 0.10), size = length(cc), replace = TRUE) ]))
...
accu_d <- accuracy(x)
write.csv(x,'W:\\cases\\Question 1\\10 percent values removed\\1NN\\MyData_catagorical_1nn_10percent.csv', row.names = TRUE)
write.table(accu_d, 'W:\\cases\\Question 1\\10 percent values removed\\1NN\\MyData_catagorical_1nn_10percent.csv', col.names=FALSE, sep=",", append=TRUE)
